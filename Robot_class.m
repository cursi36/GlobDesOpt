classdef Robot_class
    ...
    methods
        ...
        function [robot_RT] = Visualize(obj,q,name,varargin)
            robot_RT = VisualizeRobot(obj.m_type,q,obj.m_DH_table,obj.m_T_init,name,obj.m_joint_limits,varargin);
        ...
        function J = getJacobian_ee(obj,q_ctrl)
        ...
        function [T_ee,J] = getFwdKine(obj,q_ctrl,frame)
            q = (q_ctrl);
...
function [robot_RT] = VisualizeRobot(joint_types,q,DH_tab,T_base,name,limits,varargin)
...
robot_RT = SerialLink(dh,'name',char(name),'base',T_base);
% if isempty(f_prism) == 0
%     robot_RT.qlim(f_prism,:) = [zeros(length(f_prism),1), 1*ones(length(f_prism),1)];
% end
robot_RT.qlim = 0*limits;

robot_RT.plot(q','nobase')
end
