function [solver,Robot_infos,dual_arm_copy,JointType_optInfos,LinkLength_optInfos,Distance_optInfo] = readSolverConfig_Optimize_n_joints(configFile,joints_numb)
...
%                 n_joints = str2num(attributes(1).Value);
            ...
            case "joints"
                type = "";
                for n = 1:nj
                    type = type+"r"; %Initalize to all revolute
                end
                jnt_limits(:,1) = zeros(nj,1);
                jnt_limits(:,2) = zeros(nj,1);
                f = find(type == 'r');
            ...
            case "d_idx"
                ...
                if ~isempty(idx)
                    bounds(:,1) = str2num(attributes(2).Value)*ones(nj,1);
                    bounds(:,2) = str2num(attributes(3).Value)*ones(nj,1);
                    for n_idx = 1:length(idx)
            ...
            case "jnt_idx"
                ...
                if ~isempty(idx)
                    p_bounds(:,1) = str2num(attributes(2).Value)*ones(nj,1);
                    p_bounds(:,2) = str2num(attributes(4).Value)*ones(nj,1);
                    r_bounds(:,1) = str2num(attributes(3).Value)*ones(nj,1);
                    r_bounds(:,2) = str2num(attributes(5).Value)*ones(nj,1);
                    for n_idx = 1:length(idx)
